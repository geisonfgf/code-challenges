...
elements that have the smallest absolute difference between them? If there
are multiple pairs, find them all.

Input Format 
The first line of input contains a single integer, N, representing the length
of array A. 
...
(see the sample case #3 for explanation).
...
for index in xrange(len(A) - 1):
    ...
    if diff < smallest_difference:
        ...
        smallest_pairs = [(A[index], A[index + 1])]
    elif diff == smallest_difference:
        smallest_pairs.append((A[index], A[index + 1]))
